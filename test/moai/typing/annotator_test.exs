defmodule Moai.Typing.AnnotatorTest do
  use ExUnit.Case
  ...
  defp extract_type(code, symbol_table \\ Moai.Parser.Context.new([])) do
    {:ok, tokens} = Moai.Tokenizer.tokenize(code)
    {:ok, ast} = Moai.Parser.parse(tokens)

    {aast, _acc} = Moai.Parser.Ast.postwalk(ast, symbol_table, &Moai.Typing.Annotator.annotator/2)

    Moai.Typing.Annotator.extract_type(aast)
  end

  defp build_symbol_table(symbols, functions \\ %{}) do
    symbols =
      Map.new(symbols, fn {name, info} ->
        {name, Moai.Parser.Context.SymbolInfo.new(info)}
      end)
      ...
      Map.new(functions, fn {namespace, functions} ->
        {namespace,
         Map.new(functions, fn
           {name, [_ | _] = infos} ->
             {name, Enum.map(infos, &Moai.Parser.Context.SymbolInfo.new/1)}
    ...
    Moai.Parser.Context.new(symbols: symbols, functions: functions)
  end
end
