defmodule Moai.Core.Library.Number do
  ...
  def n_round(number, precision) when is_integer(precision) and precision >= 0 do
    {:ok, Float.round(number, precision)}
  end

  def n_round(_number, _precision), do: {:ok, nil}
  ...
  def n_floor(number, precision) when is_integer(precision) and precision >= 0 do
    {:ok, Float.floor(number, precision)}
  end

  def n_floor(_number, _precision), do: {:ok, nil}
  ...
  def n_ceil(number, precision) when is_integer(precision) and precision >= 0 do
    {:ok, Float.ceil(number, precision)}
  end

  def n_ceil(_number, _precision), do: {:ok, nil}
