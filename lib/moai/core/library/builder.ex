defmodule Moai.Core.Library.Builder do
  @moduledoc false
  ...
  defmacro __before_compile__(env) do
    ...
    quote location: :keep do
      @spec install_symbols(context) :: context when context: Moai.Parser.Context.t()
      def install_symbols(context) do
        Moai.Parser.Context.install_functions(context, unquote(fun_symbols))
      end

      @spec install_bindings(env) :: env when env: Moai.Core.Interpreter.Env.t()
      def install_bindings(env) do
        Moai.Core.Interpreter.Env.install_functions(env, unquote(fun_bindings))
      end
    end
  end
        ...
        Map.new(fun_meta, fn {fun_name, meta} ->
          {
            ...
            [
              Moai.Parser.Context.SymbolInfo.new(%{
    ...
    %{
      namespace =>
        Map.new(fun_meta, fn {fun_name, meta} ->
          {
            Atom.to_string(fun_name),
            Moai.Core.Interpreter.Env.Function.new(
              module,
