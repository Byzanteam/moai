defmodule Moai.Core.Library.Array do
  @moduledoc false
  ...
  def array_at([_ | _] = array, index) when is_integer(index) do
  ...
  @doc """
  Returns an array of elements in reverse order.

  ## Examples

    iex> array_reverse([1, 2, 3])
    {:ok, [3, 2, 1]}

    iex> array_reverse([])
    {:ok, []}
  """
  @fun_meta {:reverse, impl: :array_reverse, signature: [[{:var, 0}], [{:var, 0}]]}
  @spec array_reverse(array) :: {:ok, array} when array: [term()]
  def array_reverse(array) do
    {:ok, Enum.reverse(array)}
  end

  @doc """
  Returns a subset array of the given array, from start_index(zero-based) with amount number
  of elements if available.

  ## Examples

    iex> array_slice([1, 2, 3], 1, 2)
    {:ok, [2, 3]}

    iex> array_slice([1, 2, 3], 1, 10)
    {:ok, [2, 3]}

    iex> array_slice([1, 2, 3], 1, 0)
    {:ok, []}

    iex> array_slice([1, 2, 3, 4], -3, 2)
    {:ok, [2, 3]}

    iex> array_slice([1, 2, 3], 1, -1)
    {:ok, nil}
  """
  @fun_meta {:slice, impl: :array_impl, signature: [[{:var, 0}], :number, :number, [{:var, 0}]]}
  @spec array_slice(array, start :: integer(), amount :: non_neg_integer()) :: {:ok, array | nil}
        when array: [term()]
  def array_slice(array, start, amount)
      when is_integer(start) and is_integer(amount) and amount >= 0 do
    {:ok, Enum.slice(array, start, amount)}
  end

  def array_slice(_array, _start, _amount), do: {:ok, nil}
